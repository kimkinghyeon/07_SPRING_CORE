...
import com.ohgiraffers.section01.aop.MemberService;
...
public class Application {
    public static void main(String[] args) {
        ...
        /*
        리플렉션
        컴파일된 자바 코드에서 역으로 클래스를 불러 메소드 및 필드정보를 구해오는 방법을 제공
        스프링, 마이바티스, 하이버네이트, jackson 등의 라이브러리에서 사용

        리플렉션의 주의사항
        1. 오버헤드 발생 :
        성능 저하를 발생시킬 수 있다. 성능에 민감한 어플리케이션의 경우 사용하지 않는다.
        2. 캡슐화를 저해 :
        private 로 설정한 member 에 접근 가능하기 때문에 코그 기능이 저하되며 여러가지 문제 야기
        * */
        ...
        try {
            ...
            // 생성자 정보 반환
            System.out.println("=============== Account 의 필드에 대한 정보 ===============");
            constructors = Account.class.getConstructors();
            for (Constructor con : constructors) {
                System.out.println("name : " + con.getName());

                Class[] params = con.getParameterTypes();
                for (Class param : params) {
                    System.out.println("params : " + param.getTypeName());
                }
            }
            ...
            try {
                Account acc = (Account) constructors[0].newInstance("20", "110-223-123456", "1245", 10000);
        ...
        // 메소드 정보 반환
        System.out.println("=============== 메소드 정보 확인 ===============");
        Method[] method = Account.class.getMethods();
        Method getBalanceMethod = null;
        for (Method methods : method) {
            System.out.println(Modifier.toString(methods.getModifiers()) + "" +
                    methods.getReturnType().getSimpleName() + "" + methods.getName());
            if ("getBalance".equals(methods.getName())) {
                getBalanceMethod = methods;
            }
        }
